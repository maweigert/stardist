...
Conv2D, MaxPooling2D, UpSampling2D, Conv3D, MaxPooling3D, UpSampling3D, Cropping2D, Cropping3D, Concatenate, Add, Dropout, Activation, BatchNormalization, GlobalAveragePooling2D, Reshape, Dense, Permute, multiply = \
    keras_import('layers', 'Conv2D', 'MaxPooling2D', 'UpSampling2D', 'Conv3D', 'MaxPooling3D', 'UpSampling3D', 'Cropping2D', 'Cropping3D', 'Concatenate', 'Add', 'Dropout', 'Activation', 'BatchNormalization', 'GlobalAveragePooling2D', 'Reshape', 'Dense', 'Permute', 'multiply')


def _next_multiple_of(x, n):
    x = int(x)
    if x%n ==0:
        return x  
    else: 
...
def squeeze_excite_block(x, in_channels, ratio=8):
    ...
    se = Dense(in_channels // ratio, activation='relu', kernel_initializer='he_normal', use_bias=False)(se)
    se = Dense(in_channels, activation='sigmoid', kernel_initializer='he_normal', use_bias=False)(se)
    ...
    def _func(input):
        ...
        for n in range(n_depth):
            for i in range(n_conv_per_depth):
                layer = conv_block(_next_multiple_of(n_filter_base * expansion ** n, 4), *kernel_size,
                                   dropout=dropout,
            ...
            if squeeze_excite:
                layer = squeeze_excite_block(layer, _next_multiple_of(n_filter_base * expansion ** n, 4), ratio=4)
            layer = pooling(pool, name=_name("max_%s" % n))(layer)

        # middle
        for i in range(n_conv_per_depth - 1):
            layer = conv_block(_next_multiple_of(n_filter_base * expansion ** n_depth, 4), *kernel_size,
                               dropout=dropout,
                               init=kernel_init,
                               activation=activation,
                               batch_norm=batch_norm, name=_name("middle_%s" % i))(layer)
        if squeeze_excite:
            layer = squeeze_excite_block(layer, _next_multiple_of(n_filter_base * expansion ** n_depth, 4), ratio=4)
