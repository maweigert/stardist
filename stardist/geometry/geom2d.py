...
from ..lib.stardist2d import c_star_dist, c_starflow2d_map, c_starflow2d, c_starflow2d_map_float32, c_starflow2d_map_float32, c_starflow2d_map_length
...
def starflow2d_map_float32(flow, arr, mask = None, delta=.1, rounds=1, verbose=False):
    if mask is None:
        mask = np.ones(arr.shape, np.int32)
    else:
        mask = mask.astype(np.int32)
    return c_starflow2d_map_float32(flow.astype(np.float32), arr.astype(np.float32), mask, np.float32(delta), np.int32(rounds), np.int32(verbose))

def starflow2d_map_length(flow, mask = None, delta=.1, rounds=1, verbose=False):
    if mask is None:
        mask = np.ones(arr.shape, np.int32)
    else:
        mask = mask.astype(np.int32)
    return c_starflow2d_map_length(flow.astype(np.float32), mask, np.float32(delta), np.int32(rounds), np.int32(verbose))
